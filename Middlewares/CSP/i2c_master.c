	...
	for (;;)
	{
		...
		while ((packet = csp_read(conn, 100)))
		{
			if (packet->length >= 2)
			{
				...
				uint32_t ulNotified;
				while (HAL_I2C_Master_Transmit_IT(hI2C, uDevAddress, packet->data + 2, packet->length - 2) != HAL_OK)
					vTaskDelay(pdMS_TO_TICKS(1));
				xTaskNotifyWait(0L, ULONG_MAX, &ulNotified, portMAX_DELAY);
				size_t length = packet->data[1];
				csp_buffer_free(packet);
				packet = csp_buffer_get(length);
				while (HAL_I2C_Master_Receive_IT(hI2C, uDevAddress, packet->data, length) != HAL_OK);
				xTaskNotifyWait(0L, ULONG_MAX, &ulNotified, portMAX_DELAY);
