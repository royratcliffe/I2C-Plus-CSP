...
struct csp_iface_s {
  ...
  void *interface_data; // Interface data, only known/used by the interface
...
/**
   ...
   This function can be called from interface drivers (ISR) or tasks, to route
   and accept packets.

   @note EXTREMELY IMPORTANT: \a pxTaskWoken must ALWAYS be NULL if called from
   task, and ALWAYS be NON NULL if called from ISR. If this condition is met,
   this call is completely thread-safe

   This function is fire and forget, it returns void, meaning that the \a packet
   will always be either accepted or dropped, so the memory will always be
   freed.
...
*/
void csp_qfifo_write(csp_packet_t *packet, csp_iface_t *iface,
                     void *pxTaskWoken);
