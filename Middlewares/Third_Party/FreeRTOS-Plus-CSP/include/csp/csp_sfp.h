...
/**
   ...
   by chopping the data into smaller chuncks of data, that can fit into a single
   CSP message.

   SFP will add a small header to each packet, containing information about the
   transfer. SFP is usually sent over a RDP connection (which also adds a
   header),
*/
...
/**
   ...
   Data will be send in chunks of \a mtu bytes. The MTU must be small enough to
   fit into a CSP packat + SFP header + other transport headers.

   csp_sfp_recv() or csp_sfp_recv_fp() can be used at the other end to receive
   data.

   This is usefull if you wish to send data stored in flash memory or another
   location, where standard memcpy() doesn't work.
...
/**
   ...
   @param[in] first_packet First packet of a SFP transfer. Use NULL to receive
   first packet on the connection.
   @return #CSP_ERR_NONE on success, otherwise an error.
*/
