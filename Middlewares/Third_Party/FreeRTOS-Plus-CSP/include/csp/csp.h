...
typedef struct csp_conf_s {
  ...
  const char
      *hostname;     /**< Host name, returned by the #CSP_CMP_IDENT request */
  ...
  uint32_t conn_dfl_so; /**< Default connection options. Options will always be
                           or'ed onto new connections, see csp_connect() */
  uint8_t dedup; /**< Enable CSP deduplication. 0 = off, 1 = always on, 2 = only
...
/**
   ...
   This is intended for testing of CSP, in order to be able re-initialize CSP by
   calling csp_init() again.
*/
...
/**
   ...
   @param[in] socket socket to accept connections on, created by calling
   csp_socket().
   @param[in] timeout timeout in mS to wait for a connection, use
   #CSP_MAX_TIMEOUT for infinite timeout.
...
/**
   ...
   This fuction will wait on the connection's RX queue for the specified
   timeout.
   @param[in] conn connection
   @param[in] timeout timeout in mS to wait for a packet, use #CSP_MAX_TIMEOUT
...
/**
   ...
   The packet buffer is automatically freed, and cannot be used after the call
   to csp_send()
...
/**
   ...
   @note The priority of the connection will be changed. If you need to change
   it back, call csp_send_prio() again.
...
/**
   ...
   Creates a connection, send \a outbuf, wait for reply, copy reply to \a inbuf
   ...
   @param[in] inlen length of expected reply, -1 for unknown size (inbuf MUST be
   large enough), 0 for no reply.
   @param[in] opts connection options, see @ref CSP_CONNECTION_OPTIONS.
   @return 1 or reply size on success, 0 on failure (error, incoming length does
   not match, timeout)
*/
int csp_transaction_w_opts(uint8_t prio, uint16_t dst, uint8_t dst_port,
...
/**
   ...
   not match, timeout)
*/
static inline int csp_transaction(uint8_t prio, uint16_t dest, uint8_t port,
                                  uint32_t timeout, void *outbuf, int outlen,
                                  void *inbuf, int inlen) {
  return csp_transaction_w_opts(prio, dest, port, timeout, outbuf, outlen,
                                inbuf, inlen, 0);
}
...
/**
   ...
   @param[in] packet packet to send
*/
void csp_sendto(uint8_t prio, uint16_t dst, uint8_t dst_port, uint8_t src_port,
...
/**
   ...
   @param[in] opts connection options, see @ref CSP_CONNECTION_OPTIONS.
*/
void csp_sendto_reply(const csp_packet_t *request, csp_packet_t *reply,
                      uint32_t opts);

/**
   Establish outgoing connection.
   The call will return immediately, unless it is a RDP connection (#CSP_O_RDP)
   in which case it will wait until the other end acknowleges the connection
   (timeout is determined by the current connection timeout set by
   csp_rdp_set_opt()).
   @param[in] prio priority, see #csp_prio_t
   @param[in] dst Destination address
   @param[in] dst_port Destination port
   ...
   @param[in] opts connection options, see @ref CSP_CONNECTION_OPTIONS.
   @return Established connection or NULL on failure (no free connections,
   timeout).
*/
csp_conn_t *csp_connect(uint8_t prio, uint16_t dst, uint8_t dst_port,
...
/**
   ...
   @param[in] backlog max length of backlog queue. The backlog queue holds
   incoming connections, waiting to be returned by call to csp_accept().
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
...
/**
   ...
   @param[in] port port number to bind, use #CSP_ANY for all ports. Bindnig to a
   specific will take precedence over #CSP_ANY.
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
...
/**
   ...
   In order for incoming packets to routed and RDP timeouts to be checked, this
   function must be called reguarly.
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
...
/**
   ...
   If the given packet is a service-request (the destination port matches one of
   CSP service ports #csp_service_port_t), the packet will be processed by the
   specific CSP service handler. The packet will either process it or free it,
   so this function is typically called in the last "default" clause of a
   switch/case statement in a CSP listener task. In order to listen to csp
   service ports, bind your listener to the specific services ports
   #csp_service_port_t or use #CSP_ANY to all ports.
...
/**
   ...
   @param[in] node address of subsystem.
   @param[in] timeout timeout in mS to wait for replies. The function will not
   return until the timeout occurrs.
*/
...
/**
   Reboot subsystem.
   If handled by the standard CSP service handler, the reboot handler set by
   csp_sys_set_reboot() on the subsystem, will be invoked.
   @param[in] node address of subsystem.
*/
...
/**
   ...
   csp_sys_set_shutdown() on the subsystem, will be invoked.
   @param[in] node address of subsystem.
*/
...
/**
   Set RDP options.
   The RDP options are used from the connecting/client side. When a RDP
   connection is established, the client tranmits the options to the server.
   ...
   @param[in] ack_delay_count send acknowledgement for every ack_delay_count
   packets.
*/
...
/**
   ...
   @param[out] ack_delay_count send acknowledgement for every ack_delay_count
   packets
*/
