...
typedef enum {
  ...
  CSP_REBOOT =
      4, //!< Reboot, see #CSP_REBOOT_MAGIC and #CSP_REBOOT_SHUTDOWN_MAGIC
...
#define CSP_SO_SAME                                                            \
  0x8000 // Copy opts from incoming packet only apllies to csp_sendto_reply()
...
/**
   ...
   This structure is constructed to fit with all interface and protocols to
   prevent the need to copy data (zero copy).

   @note In most cases a CSP packet cannot be reused in case of send failure,
   because the lower layers may add additional data causing increased length
   (e.g. CRC32), convert the CSP id to different endian (e.g. I2C), etc.
*/
typedef struct csp_packet_s {
  ...
  union {
    ...
    struct {
      ...
      struct csp_conn_s
          *conn; // Associated connection (this is used in RDP queue)
    };
   ...
   * This must be minimum 6 bytes to accomodate CSP 2.0. But some
   * implementations require much more scratch working area for encryption for
   * example.
   *
...
/**
   ...
   The overhead is the difference between the total buffer size (returned by
   csp_buffer_size()) and the data part of the #csp_packet_t (returned by
   csp_buffer_data_size()).
*/
...
*/
typedef csp_memptr_t (*csp_memcpy_fnc_t)(csp_memptr_t, csp_const_memptr_t,
                                         size_t);

/**
   Compile check/asserts.
*/
#define CSP_STATIC_ASSERT(condition, name)                                     \
