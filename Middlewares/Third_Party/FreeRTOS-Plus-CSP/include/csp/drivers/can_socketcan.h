...
/**
   ...
   @param[in] ifname CSP interface name, use #CSP_IF_CAN_DEFAULT_NAME for
   default name.
   ...
   @param[in] promisc if \a true, receive all CAN frames. If \a false a filter
...
/**
   ...
   @return The added interface, or NULL in case of failure.
*/
csp_iface_t *csp_can_socketcan_init(const char *device, int bitrate,
                                    bool promisc);

/**
   Stop the Rx thread and free resources (testing).

   @note This will invalidate CSP, because an interface can't be removed. This
