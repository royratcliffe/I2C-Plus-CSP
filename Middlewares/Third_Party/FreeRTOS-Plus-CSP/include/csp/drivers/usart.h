...
*/
typedef void (*csp_usart_callback_t)(void *user_data, uint8_t *buf, size_t len,
                                     void *pxTaskWoken);

/**
   Opens an UART device.

   Opens the UART device and creates a thread for reading/returning data to the
   application.
...
/**
   ...
   This is a convience function for opening an UART device and adding it as an
   interface with a given name.

   @note On read failures, exit() will be called - terminating the process.

   @param[in] conf UART configuration.
   @param[in] ifname internface name (will be copied), or use NULL for default
   name.
