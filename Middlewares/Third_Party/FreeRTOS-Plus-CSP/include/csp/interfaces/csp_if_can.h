...
/**
   ...
   The \b Source and \b Destination fields must match the source and destiantion
   addressses in the CSP packet. The \b Type field is used to distinguish the
   first and subsequent frames in a fragmented CSP packet. Type is BEGIN (0) for
   the first fragment and MORE (1) for all other fragments. The \b Remain field
   indicates number of remaining fragments, and must be decremented by one for
   each fragment sent. The \b identifier field serves the same purpose as in the
   Internet Protocol, and should be an auto incrementing integer to uniquely
   separate sessions.

   Other CAN communication using a standard 11 bit identifier, can co-exist on
   the wire.
*/
...
#define CFP_SRC(id)                                                            \
  CFP_FIELD(id, CFP_HOST_SIZE + CFP_TYPE_SIZE + CFP_REMAIN_SIZE + CFP_ID_SIZE, \
            CFP_HOST_SIZE)
/** Extract destination address */
#define CFP_DST(id)                                                            \
  CFP_FIELD(id, CFP_TYPE_SIZE + CFP_REMAIN_SIZE + CFP_ID_SIZE, CFP_HOST_SIZE)
...
/** Make source */
#define CFP_MAKE_SRC(id)                                                       \
  CFP_MAKE_FIELD(id, CFP_HOST_SIZE,                                            \
                 CFP_HOST_SIZE + CFP_TYPE_SIZE + CFP_REMAIN_SIZE +             \
                     CFP_ID_SIZE)
/** Make destination */
#define CFP_MAKE_DST(id)                                                       \
  CFP_MAKE_FIELD(id, CFP_HOST_SIZE,                                            \
                 CFP_TYPE_SIZE + CFP_REMAIN_SIZE + CFP_ID_SIZE)
/** Make type */
#define CFP_MAKE_TYPE(id)                                                      \
  CFP_MAKE_FIELD(id, CFP_TYPE_SIZE, CFP_REMAIN_SIZE + CFP_ID_SIZE)
 ...
 */
#define CFP2_ID_CONN_MASK                                                      \
  ((CFP2_DST_MASK << CFP2_DST_OFFSET) |                                        \
   (CFP2_SENDER_MASK << CFP2_SENDER_OFFSET) |                                  \
   (CFP2_PRIO_MASK << CFP2_PRIO_OFFSET) | (CFP2_SC_MASK << CFP2_SC_OFFSET))
...
*/
typedef int (*csp_can_driver_tx_t)(void *driver_data, uint32_t id,
...
*/
typedef struct {
  /** CFP Identification number - same number on all fragments from same CSP
   * packet. */
...
/**
   ...
   If the MTU is not set, it will be set to the maximum value of 2042 bytes (max
   length when using CFP).
...
/**
   ...
   The function will gather the fragments into a single CSP packet and route it
   on when complete.
   ...
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
int csp_can_rx(csp_iface_t *iface, uint32_t id, const uint8_t *data,
               uint8_t dlc, int *pxTaskWoken);
