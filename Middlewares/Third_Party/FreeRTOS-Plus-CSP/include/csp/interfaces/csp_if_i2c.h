...
/**
   ...
   The function must free the frame/packet using csp_buffer_free(), if the send
   succeeds (returning #CSP_ERR_NONE).

   @param[in] driver_data driver data from #csp_iface_t
   @param[in] frame destination, length and data. This is actually a
   #csp_packet_t buffer, casted to #csp_i2c_frame_t.
...
/**
   ...
   pointing to a valid #csp_i2c_interface_data_t.
...
/**
   ...
   @note The received #csp_i2c_frame_t must actually be pointing to an
   #csp_packet_t.
