...
*/
typedef int (*csp_kiss_driver_tx_t)(void *driver_data, const uint8_t *data,
                                    size_t len);
...
/**
   ...
   If the MTU is not set, it will be set to the csp_buffer_data_size() -
   sizeof(uint32_t), to make room for the CRC32 added to the packet.

   @param[in] iface CSP interface, initialized with name and inteface_data
   pointing to a valid #csp_kiss_interface_data_t.
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
...
*/
void csp_kiss_rx(csp_iface_t *iface, const uint8_t *buf, size_t len,
                 void *pxTaskWoken);
