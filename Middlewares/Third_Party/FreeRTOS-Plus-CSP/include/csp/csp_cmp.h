...
*/
int csp_cmp(uint16_t node, uint32_t timeout, uint8_t code, int msg_size,
...
#define CMP_MESSAGE(_code, _memb)                                              \
  static inline int csp_cmp_##_memb(uint16_t node, uint32_t timeout,           \
                                    struct csp_cmp_message *msg) {             \
...
/**
   ...
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
static inline int csp_cmp_peek(uint16_t node, uint32_t timeout,
                               struct csp_cmp_message *msg) {
  return csp_cmp(node, timeout, CSP_CMP_PEEK,
                 CMP_SIZE(peek) - sizeof(msg->peek.data) + msg->peek.len, msg);
}
...
/**
   ...
   @param[in] node address of subsystem.
   @param[in] timeout timeout in mS to wait for reply..
   @param[in] msg memory address, number of bytes and the actual bytes to
   poke/write.
   @return #CSP_ERR_NONE on success, otherwise an error code.
*/
static inline int csp_cmp_poke(uint16_t node, uint32_t timeout,
                               struct csp_cmp_message *msg) {
  return csp_cmp(node, timeout, CSP_CMP_POKE,
                 CMP_SIZE(poke) - sizeof(msg->poke.data) + msg->poke.len, msg);
}
