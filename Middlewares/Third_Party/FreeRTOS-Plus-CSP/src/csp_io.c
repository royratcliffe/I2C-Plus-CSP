...
#if (CSP_USE_RDP)
  // RDP: timeout can either be 0 (for no hang poll/check) or minimum the
  // "connection timeout"
...
void csp_send_direct(csp_id_t *idout, csp_packet_t *packet,
                     csp_iface_t *routed_from) {
  ...
  while ((iface = csp_iflist_get_by_subnet(idout->dst, iface)) != NULL) {
    ...
    /* Todo: Find an elegant way to avoid making a copy when only a single
     * destination interface is found. But without looping the list twice. And
     * without using stack memory. Is this even possible? */
...
__attribute__((weak)) void csp_output_hook(csp_id_t *idout,
                                           csp_packet_t *packet,
                                           csp_iface_t *iface, uint16_t via,
                                           int from_me) {
  csp_print_packet(
      "OUT: S %u, D %u, Dp %u, Sp %u, Pr %u, Fl 0x%02X, Sz %u VIA: %s (%u)\n",
      idout->src, idout->dst, idout->dport, idout->sport, idout->pri,
      idout->flags, packet->length, iface->name,
      (via != CSP_NO_VIA_ADDRESS) ? via : idout->dst);
  return;
}

void csp_send_direct_iface(csp_id_t *idout, csp_packet_t *packet,
                           csp_iface_t *iface, uint16_t via, int from_me) {
  ...
  if (from_me) {
...
#if (CSP_USE_HMAC)
      /* Calculate and add HMAC (does not include header for backwards
       * compatability with csp1.x) */
    ...
    if (idout->flags & CSP_FCRC32) {
      /* Calculate and add CRC32 (does not include header for backwards
       * compatability with csp1.x) */
...
int csp_transaction_w_opts(uint8_t prio, uint16_t dest, uint8_t port,
  ...
  int status =
      csp_transaction_persistent(conn, timeout, outbuf, outlen, inbuf, inlen);
...
void csp_sendto(uint8_t prio, uint16_t dest, uint8_t dport, uint8_t src_port,
...
void csp_sendto_reply(const csp_packet_t *request_packet,
                      csp_packet_t *reply_packet, uint32_t opts) {
  ...
  csp_sendto(request_packet->id.pri, request_packet->id.src,
             request_packet->id.sport, request_packet->id.dport, opts,
             reply_packet);
}
