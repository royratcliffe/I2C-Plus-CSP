 ...
 * On platforms supporting max values, the semaphore is created with a max value
 * of 1, hence the naming \a binary.
 */
void csp_bin_sem_init(csp_bin_sem_t *sem);

/**
 * Wait/lock semaphore
 * @param[in] timeout timeout in mS. Use #CSP_MAX_TIMEOUT for no timeout, e.g.
 * wait forever until locked.
