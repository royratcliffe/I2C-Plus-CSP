 ...
 * This could have been done other ways, by registering functions into the csp
 * config structure to avoid a runtime comparison. But that would also clutter
 * the configuration and expose this to the user. An alternative would be a set
 * of global but non exported function pointers. That would actually be nicer,
 * but it can be done later, it works for now.
 */
...
int csp_id_is_broadcast(uint16_t addr, csp_iface_t *iface) {
  ...
  if (((addr & hostmask) == hostmask) &&
      ((addr & netmask) == (iface->addr & netmask))) {
    return 1;
  }
