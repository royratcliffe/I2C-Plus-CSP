 ...
 * implementation has been extended to include support for delayed
 * acknowledgments, to improve performance over half-duplex links.
 */
...
static int csp_rdp_close_internal(csp_conn_t *conn, uint8_t closed_by,
                                  bool send_rst);
...
static rdp_header_t *csp_rdp_header_remove(csp_packet_t *packet) {
  rdp_header_t *header =
      (rdp_header_t *)&packet->data[packet->length - sizeof(*header)];
  packet->length -= sizeof(*header);
  return header;
}
...
static inline int csp_rdp_seq_between(uint16_t seq, uint16_t start,
                                      uint16_t end) {
...
// static inline int csp_rdp_time_between(uint32_t time, uint32_t start,
// uint32_t end) {
 ...
 */
static int csp_rdp_send_cmp(csp_conn_t *conn, csp_packet_t *packet, int flags,
                            int seq_nr, int ack_nr) {
  ...
  csp_rdp_protocol("RDP %p: Send CMP S %u: syn %u, ack %u, eack %u, rst %u, "
                   "seq_nr %5u, ack_nr %5u, packet_len %u (%u)\n",
                   conn, conn->rdp.state, ((header->flags & RDP_SYN) != 0),
                   ((header->flags & RDP_ACK) != 0),
                   ((header->flags & RDP_EAK) != 0),
                   ((header->flags & RDP_RST) != 0), be16toh(header->seq_nr),
                   be16toh(header->ack_nr), packet->length,
...
static int csp_rdp_send_eack(csp_conn_t *conn) {
  ...
  unsigned int space_available =
      100 - (packet_eack->length + sizeof(rdp_header_t));
  ...
  for (i = 0; i < count; i++) {
    ...
    if (space_available >= sizeof(uint16_t)) {
      packet_eack->data16[packet_eack->length / sizeof(uint16_t)] =
          htobe16(header->seq_nr);
  ...
  return csp_rdp_send_cmp(conn, packet_eack, RDP_ACK | RDP_EAK,
                          conn->rdp.snd_nxt, conn->rdp.rcv_cur);
}
...
front:
  ...
  for (i = 0; i < count; i++) {
     ...
     * We don't hold a lock on the queue, so we require at least two spaces to
     * be free to hopefully avoid posting packets on a full queue */
...
static inline int csp_rdp_rx_queue_add(csp_conn_t *conn, csp_packet_t *packet,
                                       uint16_t seq_nr) {
...
static void csp_rdp_flush_eack(csp_conn_t *conn, csp_packet_t *eack_packet) {
  ...
  for (i = 0; i < count; i++) {
    ...
    int match = 0;
    for (j = 0; j < (int)((eack_packet->length - sizeof(rdp_header_t)) /
                          sizeof(uint16_t));
         j++) {
...
static inline bool csp_rdp_should_ack(csp_conn_t *conn) {
  ...
  uint32_t time_now = csp_get_ms();
  if (csp_rdp_time_after(time_now,
                         conn->rdp.ack_timestamp + conn->rdp.ack_timeout))
    return true;

  /* ACK if number of unacknowledged packets is greater than delay count */
  if (csp_rdp_seq_after(conn->rdp.rcv_cur,
                        conn->rdp.rcv_lsa + conn->rdp.ack_delay_count))
    return true;
...
int csp_rdp_check_ack(csp_conn_t *conn) {
  ...
  if (CSP_CONN_RXQUEUE_LEN - csp_queue_size(conn->rx_queue) <=
      2 * (int32_t)conn->rdp.window_size) {
    return CSP_ERR_NONE;
  }

  /* If more space available, only send after ack timeout or immediately if
   * delay_acks is zero */
...
static inline bool csp_rdp_is_conn_ready_for_tx(csp_conn_t *conn) {
  ...
  if (csp_rdp_seq_after(conn->rdp.snd_nxt,
                        conn->rdp.snd_una + conn->rdp.window_size - 1)) {
    return false;
  }
  return true;
}
...
void csp_rdp_check_timeouts(csp_conn_t *conn) {
  ...
  if (conn->dest_socket != NULL) {
    if (csp_rdp_time_after(time_now,
                           conn->timestamp + conn->rdp.conn_timeout)) {
      csp_rdp_error("RDP %p: Found a lost connection (now: %" PRIu32
                    ", ts: %" PRIu32 ", to: %" PRIu32 "), closing\n",
                    conn, time_now, conn->timestamp, conn->rdp.conn_timeout);
      csp_conn_close(conn, CSP_RDP_CLOSED_BY_USERSPACE |
                               CSP_RDP_CLOSED_BY_PROTOCOL |
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (csp_rdp_seq_before(be16toh(header->seq_nr), conn->rdp.snd_una)) {
      csp_rdp_protocol(
          ...
          packet->timestamp_tx, be16toh(header->seq_nr), conn->rdp.snd_una);
      csp_buffer_free(packet);
      continue;
    }
    ...
    if (csp_rdp_time_after(time_now,
                           packet->timestamp_tx + conn->rdp.packet_timeout)) {
...
bool csp_rdp_new_packet(csp_conn_t *conn, csp_packet_t *packet) {
  ...
  csp_rdp_protocol(
      ...
      conn, conn->rdp.state, ((rx_header->flags & RDP_SYN) != 0),
      ((rx_header->flags & RDP_ACK) != 0), ((rx_header->flags & RDP_EAK) != 0),
  ...
  if (rx_header->flags & RDP_RST) {
    ...
    if (rx_header->seq_nr == (conn->rdp.rcv_cur + 1)) {
      csp_rdp_protocol("RDP %p: Received RST in sequence, no more data "
                       "incoming, reply with RST\n",
                       conn);
      conn->rdp.state = RDP_CLOSE_WAIT;
      conn->timestamp = csp_get_ms();
      csp_rdp_send_cmp(conn, NULL, RDP_ACK | RDP_RST, conn->rdp.snd_nxt,
                       conn->rdp.rcv_cur);
  ...
  case RDP_CLOSED: {
    ...
    if (!(rx_header->flags & RDP_SYN)) {
      csp_rdp_protocol(
          "RDP %p: Not SYN received in CLOSED state. Discarding packet\n",
          conn);
      csp_rdp_send_cmp(conn, NULL, RDP_RST, conn->rdp.snd_nxt,
                       conn->rdp.rcv_cur);
      goto discard_close;
    }
    ...
    csp_rdp_protocol("RDP %p: window size %" PRIu32 ", conn timeout %" PRIu32
                     ", packet timeout %" PRIu32 ", delayed acks: %" PRIu32
                     ", ack timeout %" PRIu32 ", ack each %" PRIu32 " packet\n",
                     conn, conn->rdp.window_size, conn->rdp.conn_timeout,
                     conn->rdp.packet_timeout, conn->rdp.delayed_acks,
                     conn->rdp.ack_timeout, conn->rdp.ack_delay_count);

    /* Connection accepted */
    conn->rdp.state = RDP_SYN_RCVD;

    /* Send SYN/ACK */
    csp_rdp_send_cmp(conn, NULL, RDP_ACK | RDP_SYN, conn->rdp.snd_iss,
                     conn->rdp.rcv_irs);
  ...
  case RDP_SYN_SENT: {
    ...
    if ((rx_header->flags & RDP_SYN) && (rx_header->flags & RDP_ACK)) {
      ...
      /* Send ACK */
      csp_rdp_send_cmp(conn, NULL, RDP_ACK, conn->rdp.snd_nxt,
                       conn->rdp.rcv_cur);
    ...
    /* If there was no SYN in the reply, our SYN message hit an already open
     * connection This is handled by sending a RST. Normally this would be
     * followed up by a new connection attempt, however we don't have a method
     * for signaling this to the user space.
     */
    if ((rx_header->flags & RDP_ACK)) {
      csp_rdp_error(
          "RDP %p: Half-open connection found, send RST and wake Tx task\n",
          conn);
      csp_rdp_send_cmp(conn, NULL, RDP_RST, conn->rdp.snd_nxt,
                       conn->rdp.rcv_cur);
      csp_bin_sem_post(&conn->rdp.tx_wait);
    ...
    /* Otherwise we have an invalid command, such as a SYN reply to a SYN
     * command, indicating simultaneous connections, which is not possible in
     * the way CSP reserves some ports for server and some for clients.
     */
  ...
  case RDP_OPEN: {
    ...
    if (!csp_rdp_seq_between(rx_header->seq_nr, conn->rdp.rcv_cur + 1,
                             conn->rdp.rcv_cur + (conn->rdp.window_size * 2))) {
      csp_rdp_protocol(
          "RDP %p: Invalid sequence number! %u not between %u and %" PRIu32
          "\n",
          conn, rx_header->seq_nr, conn->rdp.rcv_cur + 1U,
    ...
    /* Check ACK number */
    if (!csp_rdp_seq_between(rx_header->ack_nr,
                             conn->rdp.snd_una - 1 -
                                 (conn->rdp.window_size * 2),
                             conn->rdp.snd_nxt - 1)) {
      csp_rdp_error("RDP %p: Invalid ACK number! %u not between %" PRIu32
                    " and %u\n",
                    conn, rx_header->ack_nr,
    ...
    if (conn->rdp.state == RDP_SYN_RCVD) {
      ...
      if (conn->dest_socket != NULL) {
        ...
        /* Try queueing */
        if (csp_queue_enqueue(conn->dest_socket->rx_queue, &conn, 0) ==
            CSP_QUEUE_ERROR) {
     ...
     * Unacknowledged segments are ACKed by csp_rdp_check_timeouts when the
...
discard_close:
  ...
  if (conn->dest_socket == NULL) {
    ...
    /* New connection, userspace doesn't know anything about it yet - so it can
...
retry:
  ...
  if (result == CSP_SEMAPHORE_OK) {
    ...
    if (conn->rdp.state == RDP_SYN_SENT) {
      if (retry) {
        csp_rdp_error(
            "RDP %p: Half-open connection detected, RST sent, now retrying\n",
            conn);
      ...
      csp_rdp_error(
          "RDP %p: Connection stayed half-open, even after RST and retry!\n",
          conn);
      goto error;
    }
  }
...
int csp_rdp_send(csp_conn_t *conn, csp_packet_t *packet) {
  ...
  while ((conn->rdp.state == RDP_OPEN) &&
         (csp_rdp_is_conn_ready_for_tx(conn) == false)) {
    csp_rdp_protocol(
        "RDP %p: Waiting for window update before sending seq %u\n", conn,
        conn->rdp.snd_nxt);
    if ((csp_bin_sem_wait(&conn->rdp.tx_wait, conn->rdp.conn_timeout)) !=
        CSP_SEMAPHORE_OK) {
  ...
  if (conn->rdp.state != RDP_OPEN) {
    csp_rdp_error(
        "RDP %p: ERROR cannot send, connection not open (%d) -> reset\n", conn,
        conn->rdp.state);
    return CSP_ERR_RESET;
  }
  ...
  csp_rdp_protocol("RDP %p: Sending  in S %u: syn %u, ack %u, eack %u, "
                   "rst %u, seq_nr %5u, ack_nr %5u, packet_len %u (%u)\n",
                   conn, conn->rdp.state, (tx_header->flags & RDP_SYN),
                   (tx_header->flags & RDP_ACK), (tx_header->flags & RDP_EAK),
                   (tx_header->flags & RDP_RST), be16toh(tx_header->seq_nr),
                   be16toh(tx_header->ack_nr), packet->length,
                   (unsigned int)(packet->length - sizeof(rdp_header_t)));
 ...
 * The settings are global, so be sure no other task are conflicting with your
 * settings.
 */
