...
int csp_sfp_send_own_memcpy(csp_conn_t *conn, const void *data,
                            unsigned int totalsize, unsigned int mtu,
  ...
  while (count < totalsize) {
    ...
    // csp_conn_src(conn), csp_conn_sport(conn), ((uint8_t *)data) + count,
    // size);

    /* Copy data */
    (memcpyfcn)((csp_memptr_t)(uintptr_t)packet->data,
                (csp_memptr_t)(uintptr_t)(((uint8_t *)data) + count), size);
    packet->length = size;
    ...
    /* Add SFP header */
    sfp_header = csp_sfp_header_add(
        packet); // no check, because buffer was allocated with extra size.
...
int csp_sfp_recv_fp(csp_conn_t *conn, void **return_data, int *return_datasize,
                    uint32_t timeout, csp_packet_t *first_packet) {

  *return_data = NULL; /* Allow caller to assume csp_free() can always be called
                          when dataout is non-NULL */
  *return_datasize = 0;
  ...
  do {
    ...
    if (sfp_header == NULL) {
      ...
      // __FUNCTION__, packet->id.src, packet->id.sport, packet->id.flags,
      // packet->length);
    ...
    // packet->id.src, packet->id.sport, sfp_header->offset + packet->length,
    // sfp_header->totalsize);

    /* Consistency check */
    if (sfp_header->offset != data_offset) {
      // csp_print("%s: %u:%u, invalid message, offset %" PRIu32 " (expected %"
      // PRIu32 "), length: %u, totalsize %" PRIu32 "\n", __FUNCTION__,
      // packet->id.src, packet->id.sport, sfp_header->offset, data_offset,
      // packet->length, sfp_header->totalsize);
      csp_buffer_free(packet);
    ...
    if (data == NULL) {
      ...
      if (data == NULL) {
        ...
        // packet->id.src, packet->id.sport, datasize);
        csp_buffer_free(packet);
    ...
    /* Consistency check */
    if (((data_offset + packet->length) > datasize) ||
        (datasize != sfp_header->totalsize)) {
      // csp_print("%s: %u:%u, invalid size, sfp.offset: %" PRIu32 ", length:
      // %u, total: %" PRIu32 " / %" PRIu32 "\n", __FUNCTION__, packet->id.src,
      // packet->id.sport, sfp_header->offset, packet->length, datasize,
      // sfp_header->totalsize);
      csp_buffer_free(packet);
