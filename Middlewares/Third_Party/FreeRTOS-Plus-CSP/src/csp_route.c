 ...
 */
static int csp_route_security_check(uint32_t security_opts, csp_iface_t *iface,
                                    csp_packet_t *packet) {

  /* CRC32 verified packet */
  if (packet->id.flags & CSP_FCRC32) {
    /* Verify CRC32 (does not include header for backwards compatability with
     * csp1.x) */
  ...
  if (packet->id.flags & CSP_FHMAC) {
    /* Verify HMAC (does not include header for backwards compatability with
     * csp1.x) */
...
__attribute__((weak)) void csp_input_hook(csp_iface_t *iface,
                                          csp_packet_t *packet) {
  csp_print_packet(
      "INP: S %u, D %u, Dp %u, Sp %u, Pr %u, Fl 0x%02X, Sz %" PRIu16
      " VIA: %s\n",
      packet->id.src, packet->id.dst, packet->id.dport, packet->id.sport,
      packet->id.pri, packet->id.flags, packet->length, iface->name);
}
...
int csp_route_work(void) {
  ...
  int is_to_me = ((input.iface->addr == packet->id.dst) ||
