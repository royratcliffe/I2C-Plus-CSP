...
static csp_static_queue_t csp_promisc_queue_static
    __attribute__((section(".noinit")));
char csp_promisc_queue_buffer[sizeof(csp_packet_t *) * CSP_CONN_RXQUEUE_LEN]
    __attribute__((section(".noinit")));
...
int csp_promisc_enable(unsigned int queue_size) {
  ...
  csp_promisc_queue = csp_queue_create_static(
      CSP_CONN_RXQUEUE_LEN, sizeof(csp_packet_t *), csp_promisc_queue_buffer,
      &csp_promisc_queue_static);
...
void csp_promisc_disable(void) { csp_promisc_enabled = 0; }
...
void csp_promisc_add(csp_packet_t *packet) {
  ...
  if (csp_promisc_queue != NULL) {
    ...
    if (packet_copy != NULL) {
      if (csp_queue_enqueue(csp_promisc_queue, &packet_copy, 0) !=
          CSP_QUEUE_OK) {
        csp_dbg_conn_ovf++;
