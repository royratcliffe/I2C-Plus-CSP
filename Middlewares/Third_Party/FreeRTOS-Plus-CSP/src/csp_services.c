...
int csp_ping(uint16_t node, uint32_t timeout, unsigned int size,
  ...
  /* Open connection */
  csp_conn_t *conn =
      csp_connect(CSP_PRIO_NORM, node, CSP_PING, timeout, conn_options);
  if (conn == NULL)
    return -1;
...
void csp_ps(uint16_t node, uint32_t timeout) {
  ...
  while (1) {
    ...
    const unsigned int length = (packet->length < csp_buffer_data_size())
                                    ? packet->length
                                    : (csp_buffer_data_size() - 1);
...
int csp_get_memfree(uint16_t node, uint32_t timeout, uint32_t *size) {
  ...
  int status = csp_transaction(CSP_PRIO_NORM, node, CSP_MEMFREE, timeout, NULL,
                               0, size, sizeof(*size));
...
int csp_get_buf_free(uint16_t node, uint32_t timeout, uint32_t *size) {
  ...
  int status = csp_transaction(CSP_PRIO_NORM, node, CSP_BUF_FREE, timeout, NULL,
                               0, size, sizeof(*size));
  if (status == sizeof(*size)) {
    *size = be32toh(*size);
    return CSP_ERR_NONE;
