...
static char
    tx_queue_static_data[sizeof(csp_packet_t *) * CSP_RDP_MAX_WINDOW * 2];

static csp_queue_handle_t rx_queue;
static csp_static_queue_t rx_queue_static; /* Static storage for rx queue */
static char
    rx_queue_static_data[sizeof(csp_packet_t *) * CSP_RDP_MAX_WINDOW * 2];

void csp_rdp_queue_init(void) {

  /* Create TX queue */
  tx_queue =
      csp_queue_create_static(CSP_RDP_MAX_WINDOW * 2, sizeof(csp_packet_t *),
                              tx_queue_static_data, &tx_queue_static);

  /* Create RX queue */
  rx_queue =
      csp_queue_create_static(CSP_RDP_MAX_WINDOW * 2, sizeof(csp_packet_t *),
                              rx_queue_static_data, &rx_queue_static);
}
...
int csp_rdp_queue_tx_size(void) { return csp_queue_size(tx_queue); }
...
int csp_rdp_queue_rx_size(void) { return csp_queue_size(rx_queue); }
