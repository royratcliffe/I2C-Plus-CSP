 ...
 */
#ifdef __AVR__
static uint32_t wrap_32bit_memcpy(uint32_t to, const uint32_t from,
                                  size_t size) {
  return (uint32_t)(uintptr_t)memcpy((void *)(uintptr_t)to,
                                     (const void *)(uintptr_t)from, size);
}
...
void csp_cmp_set_memcpy(csp_memcpy_fnc_t fnc) { csp_cmp_memcpy_fnc = fnc; }
...
static int do_cmp_route_set_v1(struct csp_cmp_message *cmp) {
  ...
  if (csp_rtable_set(cmp->route_set_v1.dest_node, csp_id_get_host_bits(), ifc,
                     cmp->route_set_v1.next_hop_via) != CSP_ERR_NONE) {
    return CSP_ERR_INVAL;
  }
...
static int do_cmp_route_set_v2(struct csp_cmp_message *cmp) {
  ...
  if (csp_rtable_set(be16toh(cmp->route_set_v2.dest_node),
                     be16toh(cmp->route_set_v2.netmask), ifc,
                     be16toh(cmp->route_set_v2.next_hop_via)) != CSP_ERR_NONE) {
    return CSP_ERR_INVAL;
  }
