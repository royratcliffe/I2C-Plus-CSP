...
static int csp_rtable_parse(const char *rtable, int dry_run) {
  ...
  while ((str) && (strlen(str) > 1)) {
    ...
    if ((address > csp_id_get_max_nodeid()) ||
        (netmask > (int)csp_id_get_host_bits()) || (ifc == NULL)) {
...
int csp_rtable_load(const char *rtable) { return csp_rtable_parse(rtable, 0); }

int csp_rtable_check(const char *rtable) { return csp_rtable_parse(rtable, 1); }
...
static bool csp_rtable_save_route(void *vctx, csp_route_t *route) {
  ...
  int res =
      snprintf(ctx->buffer + ctx->len, remain_buf_size, "%s%u%s %s%s", sep,
               route->address, mask_str, route->iface->name, via_str);
...
int csp_rtable_save(char *buffer, size_t maxlen) {
  csp_rtable_save_ctx_t ctx = {
      .len = 0, .buffer = buffer, .maxlen = maxlen, .error = CSP_ERR_NONE};
  buffer[0] = 0;
