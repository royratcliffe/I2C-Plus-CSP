...
void csp_iflist_set_default(csp_iface_t *interface) { dfl_if = interface; }

csp_iface_t *csp_iflist_get_default(void) { return dfl_if; }
...
int csp_iflist_is_within_subnet(uint16_t addr, csp_iface_t *ifc) {
  ...
  uint16_t netmask = ((1 << ifc->netmask) - 1)
                     << (csp_id_get_host_bits() - ifc->netmask);
...
int csp_iflist_add(csp_iface_t *ifc) {
  ...
  if (interfaces == NULL) {
    ...
    for (csp_iface_t *i = interfaces; i != NULL; i = i->next) {
      if ((i == ifc) ||
          (strncmp(ifc->name, i->name, CSP_IFLIST_NAME_MAX) == 0)) {
...
csp_iface_t *csp_iflist_get(void) { return interfaces; }
...
void csp_iflist_print(void) {
  ...
  while (i) {
    ...
    csp_print("%-10s addr: %" PRIu16 " netmask: %" PRIu16 " mtu: %" PRIu16
              "\r\n"
              "           tx: %05" PRIu32 " rx: %05" PRIu32 " txe: %05" PRIu32
              " rxe: %05" PRIu32 "\r\n"
              "           drop: %05" PRIu32 " autherr: %05" PRIu32
              " frame: %05" PRIu32 "\r\n"
              "           txb: %" PRIu32 " (%" PRIu32 "%c) rxb: %" PRIu32
              " (%" PRIu32 "%c) \r\n\r\n",
              i->name, i->addr, i->netmask, i->mtu, i->tx, i->rx, i->tx_error,
              i->rx_error, i->drop, i->autherr, i->frame, i->txbytes, tx,
              tx_postfix, i->rxbytes, rx, rx_postfix);
    i = i->next;
  }
}
