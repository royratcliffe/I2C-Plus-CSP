...
void csp_conn_init(void) {
  ...
  for (int i = 0; i < CSP_CONN_MAX; i++) {
    ...
    conn->rx_queue = csp_queue_create_static(
        CSP_CONN_RXQUEUE_LEN, sizeof(csp_packet_t *),
        conn->rx_queue_static_data, &conn->rx_queue_static);
...
csp_conn_t *csp_conn_find_existing(csp_id_t *id) {
  ...
  for (int i = 0; i < CSP_CONN_MAX; i++) {
    ...
    /**
     * This search looks verbose, Instead of a big if statement, it is written
     * out as conditions. This has been done for clarity. The least likely check
     * is put first for runtime speed improvement. Also this is written using
     * explicit fields, not bitmasks, in order to improve portability and dual
     * use between different header formats.
     */
...
csp_conn_t *csp_conn_allocate(csp_conn_type_t type) {
  ...
  for (int j = 0; j < CSP_CONN_MAX; j++) {
    ...
    if (atomic_compare_exchange_strong(&arr_conn[i].state, &expected,
                                       CONN_OPEN)) {
      conn = &arr_conn[i];
...
csp_conn_t *csp_connect(uint8_t prio, uint16_t dest, uint8_t dport,
   ...
   * This means that for this outgoing connection, we accept the answer coming
   * to whatever address the outgoing interface has. CSP does not support
   * "source address" on outgoing connections so the outgoing source address
   * will be automatically applied after outgoing routing selects which
   * interface the packet will leavve from */
  incoming_id.dst = 0;
  outgoing_id.src = 0;
...
int csp_conn_dport(csp_conn_t *conn) { return conn->idin.dport; }

int csp_conn_sport(csp_conn_t *conn) { return conn->idin.sport; }

int csp_conn_dst(csp_conn_t *conn) { return conn->idin.dst; }

int csp_conn_src(csp_conn_t *conn) { return conn->idin.src; }

int csp_conn_flags(csp_conn_t *conn) { return conn->idin.flags; }
...
void csp_conn_print_table(void) {
  ...
  for (unsigned int i = 0; i < CSP_CONN_MAX; i++) {
    ...
    csp_print("[%02u %p] S:%u, %u -> %u, %u -> %u (%u) fl %x\r\n", i, conn,
              ...
              conn->idin.sport, conn->sport_outgoing, conn->idin.flags);
#if (CSP_USE_RDP)
    if (conn->idin.flags & CSP_FRDP) {
      csp_print("\tRDP: S:%d (closed by 0x%x), rcv %u, snd %u, win %" PRIu32
                "\n",
                conn->rdp.state, conn->rdp.closed_by, conn->rdp.rcv_cur,
                conn->rdp.snd_una, conn->rdp.window_size);
    }
#endif
  }
}
...
int csp_conn_print_table_str(char *str_buf, int str_size) {
  ...
  for (unsigned int i = start; i < CSP_CONN_MAX; i++) {
    ...
    snprintf(buf, sizeof(buf), "[%02u %p] S:%u, %u -> %u, %u -> %u (%u)\n", i,
             conn, conn->state, conn->idin.src, conn->idin.dst,
