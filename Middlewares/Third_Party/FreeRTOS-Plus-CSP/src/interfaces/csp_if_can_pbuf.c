...
void csp_can_pbuf_free(csp_can_interface_data_t *ifdata, csp_packet_t *buffer,
                       int buf_free, int *task_woken) {
...
csp_packet_t *csp_can_pbuf_new(csp_can_interface_data_t *ifdata, uint32_t id,
                               int *task_woken) {
  ...
  csp_packet_t *packet =
      (task_woken) ? csp_buffer_get_isr(0) : csp_buffer_get(0);
  if (packet == NULL) {
    return NULL;
  }
...
csp_packet_t *csp_can_pbuf_find(csp_can_interface_data_t *ifdata, uint32_t id,
                                uint32_t mask, int *task_woken) {
