  ...
  while (len--) {
    ...
    case KISS_MODE_NOT_STARTED:
      ...
      if (ifdata->rx_packet == NULL) {
        ifdata->rx_packet =
            pxTaskWoken ? csp_buffer_get_isr(0)
                        : csp_buffer_get(0); // CSP only supports one size
      }
    ...
    case KISS_MODE_STARTED:
      ...
      if (inputbyte == FEND) {
        ...
        if (ifdata->rx_length > 0) {
          ...
          /* Send back into CSP, notice calling from task so last argument must
           * be NULL! */
...
int csp_kiss_add_interface(csp_iface_t *iface) {
  ...
  const unsigned int max_data_size =
      ...
      sizeof(uint32_t); // compensate for the added CRC32
