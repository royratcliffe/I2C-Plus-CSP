...
int csp_can1_rx(csp_iface_t *iface, uint32_t id, const uint8_t *data,
                uint8_t dlc, int *task_woken) {
  ...
  csp_packet_t *packet =
      csp_can_pbuf_find(ifdata, id, CFP_ID_CONN_MASK, task_woken);
  if (packet == NULL) {
...
int csp_can1_tx(csp_iface_t *iface, uint16_t via, csp_packet_t *packet,
  ...
  /* Get an unique CFP id - this should be locked to prevent access from
   * multiple tasks */
   ...
   */
  can_id = (CFP_MAKE_SRC(packet->id.src) | CFP_MAKE_DST(dest) |
            CFP_MAKE_ID(ident) | CFP_MAKE_TYPE(CFP_BEGIN) |
            CFP_MAKE_REMAIN((packet->length + CFP1_DATA_OFFSET - 1) /
                            CAN_FRAME_SIZE));
  ...
  memcpy(frame_buf + CFP1_CSP_HEADER_OFFSET, packet->frame_begin,
         CFP1_CSP_HEADER_SIZE);
  ...
  data_bytes = (packet->length <= CFP1_DATA_SIZE_BEGIN) ? packet->length
  ...
  /* Send first frame */
  if ((tx_func)(iface->driver_data, can_id, frame_buf,
                CFP1_DATA_OFFSET + data_bytes) != CSP_ERR_NONE) {
    iface->tx_error++;
  ...
  while (tx_count < packet->length) {
    ...
    data_bytes = (packet->length - tx_count >= CAN_FRAME_SIZE)
                     ? CAN_FRAME_SIZE
                     : packet->length - tx_count;

    /* Prepare identifier */
    can_id = (CFP_MAKE_SRC(packet->id.src) | CFP_MAKE_DST(dest) |
              CFP_MAKE_ID(ident) | CFP_MAKE_TYPE(CFP_MORE) |
              CFP_MAKE_REMAIN((packet->length - tx_count - data_bytes +
                               CAN_FRAME_SIZE - 1) /
                              CAN_FRAME_SIZE));
    ...
    /* Send frame */
    if ((tx_func)(iface->driver_data, can_id,
                  packet->data + tx_count - data_bytes,
                  data_bytes) != CSP_ERR_NONE) {
      iface->tx_error++;
      /* Does not free on return */
      return CSP_ERR_DRIVER;
    }
...
int csp_can2_rx(csp_iface_t *iface, uint32_t id, const uint8_t *data,
                uint8_t dlc, int *task_woken) {
  ...
  /* Bind incoming frame to a packet buffer */
  csp_packet_t *packet =
      csp_can_pbuf_find(ifdata, id, CFP2_ID_CONN_MASK, task_woken);
  if (packet == NULL) {
...
int csp_can2_tx(csp_iface_t *iface, uint16_t via, csp_packet_t *packet,
  ...
  uint32_t
      frame_buf_mem[(CAN_FRAME_SIZE + sizeof(uint32_t) - 1) / sizeof(uint32_t)];
  ...
  *header_extension = htobe32(*header_extension);
  ...
  if ((ifdata->tx_func)(iface->driver_data, can_id, frame_buf, frame_buf_inp) !=
      CSP_ERR_NONE) {
    iface->tx_error++;
    /* Does not free on return */
    return CSP_ERR_DRIVER;
  ...
  while (tx_count < packet->length) {
    ...
    /* Send frame */
    if ((ifdata->tx_func)(iface->driver_data, can_id, packet->data + tx_count,
                          data_bytes) != CSP_ERR_NONE) {
      iface->tx_error++;
      /* Does not free on return */
      return CSP_ERR_DRIVER;
