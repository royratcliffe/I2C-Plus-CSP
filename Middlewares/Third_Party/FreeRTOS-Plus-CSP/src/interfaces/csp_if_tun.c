...
__attribute__((weak)) int csp_crypto_decrypt(uint8_t *ciphertext_in,
                                             uint8_t ciphertext_len,
                                             uint8_t *msg_out) {
  return -1;
}

__attribute__((weak)) int csp_crypto_encrypt(uint8_t *msg_begin,
                                             uint8_t msg_len,
...
static int csp_if_tun_tx(csp_iface_t *iface, uint16_t via, csp_packet_t *packet,
  ...
  if (packet->id.dst == ifconf->tun_src) {
...
#if 1
    int length = csp_crypto_decrypt(packet->data, packet->length,
    ...
    // csp_hex_dump("new frame", new_packet->frame_begin,
    // new_packet->frame_length + 16);
    ...
    new_packet->length = csp_crypto_encrypt(
        packet->frame_begin, packet->frame_length, new_packet->data);
#else
    /* Encapsulate */
...
void csp_if_tun_init(csp_iface_t *iface, csp_if_tun_conf_t *ifconf) {
  ...
  /* Regsiter interface */
  iface->name = "TUN", iface->nexthop = csp_if_tun_tx, csp_iflist_add(iface);
}
