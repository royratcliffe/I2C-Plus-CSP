...
int csp_i2c_tx(csp_iface_t *iface, uint16_t via, csp_packet_t *packet,
  ...
  /* There is only 7 address bits available on CSP, so use the lower 7 bits for
   * destination */
...
void csp_i2c_rx(csp_iface_t *iface, csp_packet_t *packet, void *pxTaskWoken) {
  ...
  if (packet->frame_length < sizeof(uint32_t)) {
    iface->frame++;
    (pxTaskWoken != NULL) ? csp_buffer_free_isr(packet)
  ...
  if (packet->frame_length >
      csp_buffer_data_size()) { // consistency check, should never happen
    iface->rx_error++;
    (pxTaskWoken != NULL) ? csp_buffer_free_isr(packet)
                          : csp_buffer_free(packet);
    return;
