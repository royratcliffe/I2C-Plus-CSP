...
static int csp_hmac_init(hmac_state *hmac, const uint8_t *key,
                         uint32_t keylen) {
  uint32_t i;
  ...
  if (keylen > CSP_SHA1_BLOCKSIZE) {
    ...
    if (CSP_SHA1_DIGESTSIZE < CSP_SHA1_BLOCKSIZE)
      memset((hmac->key) + CSP_SHA1_DIGESTSIZE, 0,
             (CSP_SHA1_BLOCKSIZE - CSP_SHA1_DIGESTSIZE));
  } else {
...
static int csp_hmac_process(hmac_state *hmac, const uint8_t *in,
                            uint32_t inlen) {
...
int csp_hmac_append(csp_packet_t *packet, bool include_header) {
  ...
  if (include_header) {
    ...
    csp_hmac_memory(csp_hmac_key, sizeof(csp_hmac_key), packet->frame_begin,
                    packet->frame_length, hmac);
    memcpy(&packet->frame_begin[packet->frame_length], hmac, CSP_HMAC_LENGTH);
    packet->frame_length += CSP_HMAC_LENGTH;

  } else {

    csp_hmac_memory(csp_hmac_key, sizeof(csp_hmac_key), packet->data,
                    packet->length, hmac);
...
int csp_hmac_verify(csp_packet_t *packet, bool include_header) {
  ...
  if (include_header) {
    ...
    csp_hmac_memory(csp_hmac_key, sizeof(csp_hmac_key), packet->frame_begin,
                    packet->frame_length - CSP_HMAC_LENGTH, hmac);

    /* Compare calculated HMAC with packet header */
    if (memcmp(&packet->frame_begin[packet->frame_length] - CSP_HMAC_LENGTH,
               hmac, CSP_HMAC_LENGTH) != 0) {
      /* HMAC failed */
      return CSP_ERR_HMAC;
    }
    ...
    csp_hmac_memory(csp_hmac_key, sizeof(csp_hmac_key), packet->data,
                    packet->length - CSP_HMAC_LENGTH, hmac);

    /* Compare calculated HMAC with packet header */
    if (memcmp(&packet->data[packet->length] - CSP_HMAC_LENGTH, hmac,
               CSP_HMAC_LENGTH) != 0) {
      /* HMAC failed */
      return CSP_ERR_HMAC;
    }
