...
#define SKBUF_SIZE                                                             \
  CSP_BUFFER_ALIGN *                                                           \
      ((sizeof(csp_skbf_t) + (CSP_BUFFER_ALIGN - 1)) / CSP_BUFFER_ALIGN)
...
void csp_buffer_init(void) {
   ...
   * This is marked as .noinit, because csp buffers can never be assumed zeroed
   * out Putting this section in a separate non .bss area, saves some boot time
   */
  ...
  csp_buffers =
      csp_queue_create_static(CSP_BUFFER_COUNT, sizeof(csp_skbf_t *),
                              csp_buffer_queue_data, &csp_buffers_queue);
...
void csp_buffer_free(void *packet) {
  ...
  csp_skbf_t *buf =
      (void *)(((uint8_t *)packet) + sizeof(csp_packet_t) - sizeof(csp_skbf_t));
...
int csp_buffer_remaining(void) { return csp_queue_size(csp_buffers); }

size_t csp_buffer_size(void) { return sizeof(csp_packet_t); }
