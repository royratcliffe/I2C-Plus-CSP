...
int csp_rtable_set_internal(uint16_t address, uint16_t netmask,
                            csp_iface_t *ifc, uint16_t via) {
...
void csp_rtable_free(void) { memset(rtable, 0, sizeof(rtable)); }

void csp_rtable_clear(void) { csp_rtable_free(); }

int csp_rtable_set(uint16_t address, int netmask, csp_iface_t *ifc,
                   uint16_t via) {
